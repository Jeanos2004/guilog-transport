...
export default function StudentHeader({ 
  ...
  useEffect(() => {
    const fetchNotifs = async () => {
      ...
      if (!user) return;
      const profileData = await studentDb.getProfile(user.uid);
      const enrolledIds = profileData?.enrolledCourses || [];
      const courses = await studentDb.getCourses();
      const enrolledCourses = courses.filter(c => enrolledIds.includes(c.id));
      const allSessions: any[] = [];
      enrolledCourses.forEach(c => c.modules.forEach(m => m.sessions.forEach(s => allSessions.push({course: c, session: s}))));
